...
import {WatchesAction , WatchesMinusAction ,WatchesRemoveAction} from '../../redux/actions/watchesAction'
import {useDispatch} from 'react-redux'

const Item = ({id,title,image,oldPrice,newPrice,count,totalPrice,item}) => {
  ...
  return (
    <div key={id} className='modal__item modal-item'>
      ...
      <div className='modal-item__right'>
        <div className='modal-item__cart'>
          <button onClick={MinusOnClick} className='modal-item__minus'>-</button>
          <div className='modal-item__count'>{count}</div>
          <button onClick={AddOnClick} className='modal-item__plus'>+</button>
          <span className='modal-item__total'>Rp {totalPrice}</span>
          <button onClick={RemoveOnClick} className='modal-item__remove'>
            <svg
              width='24'
              height='24'
              viewBox='0 0 24 24'
